...
int sc_fragment_packet(uint16_t can_id, const uint8_t *payload, size_t len, spacecan_frame_t *frames, size_t *frame_count) {
  ...
  *frame_count = needed;
  ...
  for (size_t i = 0; i < needed; i++) {
    ...
    size_t chunk = (len - offset >= MAX_CHUNK_LEN) ? MAX_CHUNK_LEN : (len - offset);
...
int sc_reassembly_packets(spacecan_reassembly_ctx_t *ctx, const spacecan_frame_t *frame, uint32_t now_ms, uint8_t *out_buffer, size_t *out_len) {
